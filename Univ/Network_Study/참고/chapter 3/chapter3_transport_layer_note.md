...
- 개념
  - 손실되거나 오류 난 패킷만 선택적으로 재전송하는 파이프라인 ARQ 방식
  - GBN보다 효율적이지만 송신자/수신자 관리가 더 복잡하다

- sender
  - 패킷별 ACK 관리
    - 각 패킷이 개별적으로 확인되었는지 추적해야 함
  - 패킷별 타이머 관리
    - 손실된 패킷만 재전송하려면 각 패킷마다 timeout을 따로 관리해야 함
  - 손실된 것만 골라 재전송
    - 이미 정상 수신된 패킷은 다시 보내지 않음

- receiver
  - out-of-order 패킷도 buffer에 저장 가능
    - 순서가 어긋나도 버리지 않고 저장해 둠
  - 각 패킷별 개별 ACK
    - 받은 패킷 번호마다 각각 ACK 전송
  - 빠진 것이 채워지면 순서대로 상위 계층에 전달
    - 중간에 비어 있던 번호가 도착하면 저장해 두었던 패킷들도 연속 전달 가능
...
- 단점
  - 송신자: 패킷별 상태/타이머 관리 필요
  - 수신자: out-of-order 버퍼링 필요
  - 구현 복잡
  - 윈도우 및 시퀀스 번호 관리가 더 까다로움
